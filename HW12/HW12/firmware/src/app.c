...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_MOUSE_EMULATE:
            ...
            //IIR
            xIIR = alpha * xIIR + beta*x_acc; //IIR
            yIIR = alpha * yIIR + beta*y_acc; //IIR
            
            
            sprintf(m,"accel x,y,z %f, %f, %d  ",xIIR,yIIR,z_acc);
            _CP0_SET_COUNT(0);
            LCD_print(m,0,5,ILI9341_WHITE,ILI9341_BLACK);
            LCD_drawbar('h',100*xIIR/32767,100,5,120,160,ILI9341_CYAN,ILI9341_BLACK);//need to edit LCD_drawbar to go in directions besides right
            LCD_drawbar('v',100*yIIR/32767,100,5,120,160,ILI9341_CYAN,ILI9341_BLACK);//need to edit LCD_drawbar to go in directions besides right
            ...
            int8_t xcoor = xIIR/600;
            int8_t ycoor = yIIR/600;
            //if (move > 10){ //due to the latency from using the LCD screen, it doesn't need this delay
                appData.xCoordinate = (int8_t) xcoor;
                appData.yCoordinate = (int8_t) ycoor;
            ...
            * 
            */
            sprintf(m,"mouse x: %d, y: %d",appData.xCoordinate,appData.yCoordinate);
            LCD_print(m,0,20,ILI9341_WHITE,ILI9341_BLACK);
